...
const persistor = new Persistor('Mini-Conf-Demo-Papers');
...
const start = () => {
    ...
    d3.json('demo_papers.json').then(papers => {
...
const keyword = kw => `<a href="demo_papers.html?filter=keywords&search=${kw}"
                       class="text-secondary text-decoration-none">${kw.toLowerCase()}</a>`

const card_image = (openreview, show) => {
    if (show) return ` <center><img class="lazy-load-img cards_img" data-src="static/images/acl2020/demo_papers/${openreview.id}.png" width="80%"/></center>`
    else return ''
}
...
const card_cal = (openreview, i) => `<a class="text-muted" href="webcal://iclr.github.io/iclr-images/calendars/demo_poster_${openreview.forum}.${i}.ics">${card_icon_cal}</a>`
...
const card_html = openreview => `
        <div class="pp-card pp-mode-` + render_mode + ` ">
            ...
            <div class="checkbox-paper ${openreview.content.read ? 'selected' : ''}" style="display: block;position: absolute; bottom:35px;left: 35px;">✓</div>    
                <a href="demo_poster_${openreview.id}.html"
                target="_blank"
                   class="text-muted">
